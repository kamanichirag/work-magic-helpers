...
export const VendorAssessmentTab = ({
  formData,
  handleChange
}: VendorAssessmentTabProps) => {
    ...
    questions: {
      ...
      <div className="border rounded-md overflow-hidden">
        <Table>
          ...
          <TableBody>
            {questions.map((q, index) => <TableRow key={`${section}-${q.field}`} className="border-b">
                {index === 0 ? <TableCell className="font-medium align-middle text-center border-r" rowSpan={questions.length}>
                ...
                <TableCell className="align-top border-r p-4">
                  {q.type === "text" ? <Textarea value={(assessment[section as keyof typeof assessment] as any)[q.field].response} onChange={e => handleTextResponseChange(section, q.field, e.target.value)} placeholder="Enter response" className="w-full" rows={5} /> : <div className="flex flex-col space-y-2">
                      <div className="flex items-center space-x-2">
                        <Checkbox id={`${section}-${q.field}-yes`} checked={(assessment[section as keyof typeof assessment] as any)[q.field].response === "Yes"} onCheckedChange={() => handleCheckboxResponseChange(section, q.field, "Yes")} />
                        <Label htmlFor={`${section}-${q.field}-yes`}>Yes</Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Checkbox id={`${section}-${q.field}-no`} checked={(assessment[section as keyof typeof assessment] as any)[q.field].response === "No"} onCheckedChange={() => handleCheckboxResponseChange(section, q.field, "No")} />
                        <Label htmlFor={`${section}-${q.field}-no`}>No</Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Checkbox id={`${section}-${q.field}-na`} checked={(assessment[section as keyof typeof assessment] as any)[q.field].response === "Not Applicable"} onCheckedChange={() => handleCheckboxResponseChange(section, q.field, "Not Applicable")} />
                        <Label htmlFor={`${section}-${q.field}-na`}>Not Applicable</Label>
                      </div>
                    </div>}
                </TableCell>
                <TableCell className="align-top p-4">
                  <Textarea value={(assessment[section as keyof typeof assessment] as any)[q.field].remarks} onChange={e => handleRemarksChange(section, q.field, e.target.value)} placeholder="Enter remarks" className="min-h-[80px] w-full" />
                </TableCell>
              </TableRow>)}
          </TableBody>
        </Table>
  ...
  return <div>
      ...
      <div className="space-y-8">
        <AssessmentTable title="Organization and Personnel" section="department" questions={[{
        ...
        <AssessmentTable title="Facility" section="facility" questions={[{
        field: "security",
        ...
        <AssessmentTable title="Labeling" section="labeling" questions={[{
        field: "overLabelling",
        ...
        <AssessmentTable title="Comparator Sourcing" section="comparatorSourcing" questions={[{
        ...
        <AssessmentTable title="Records And Reports" section="recordsAndReports" questions={[{
